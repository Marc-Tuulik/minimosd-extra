...
*/
	static const PROGMEM mavlink_msg_entry_t mavlink_message_crcs[] = MAVLINK_MESSAGE_CRCS;
        /*
	...
	*/
        ...
        while (low < high) {
            ...
            if (msgid < pgm_read_dword(&mavlink_message_crcs[mid].msgid)) {
                high = mid-1;
                continue;
            }
            if (msgid > pgm_read_dword(&mavlink_message_crcs[mid].msgid)) {
...
MAVLINK_HELPER uint8_t mavlink_get_crc_extra(const mavlink_message_t *msg)
{
	...
	return e?pgm_read_byte(&e->crc_extra):0;
}
...
{
	const mavlink_msg_entry_t *e = mavlink_get_msg_entry(msg->msgid);
	return e?pgm_read_byte(&e->msg_len):0;
}
...
/* Support shorter buffers than the
   default maximum packet size */
	...
	case MAVLINK_PARSE_STATE_GOT_PAYLOAD: {
		...
		uint8_t crc_extra = e?pgm_read_byte(&e->crc_extra):0;
	...
	case MAVLINK_PARSE_STATE_SIGNATURE_WAIT:
		...
		if (status->signature_wait == 0) {
			...
			bool sig_ok = true; //mavlink_signature_check(status->signing, status->signing_streams, rxmsg);
			if (!sig_ok &&
